// <auto-generated />
using System;
using EchoesOfTheRealms;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace EchoesOfTheRealmsShared.Migrations
{
    [DbContext(typeof(EotRContext))]
    [Migration("20260212074816_Mig120226-EndAttack")]
    partial class Mig120226EndAttack
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.22")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("CharacterEquipment", b =>
                {
                    b.Property<long>("CharactersId")
                        .HasColumnType("bigint");

                    b.Property<long>("EquipmentsId")
                        .HasColumnType("bigint");

                    b.HasKey("CharactersId", "EquipmentsId");

                    b.HasIndex("EquipmentsId");

                    b.ToTable("CharacterEquipment");
                });

            modelBuilder.Entity("CharacterItem", b =>
                {
                    b.Property<long>("CharactersId")
                        .HasColumnType("bigint");

                    b.Property<long>("ItemsId")
                        .HasColumnType("bigint");

                    b.HasKey("CharactersId", "ItemsId");

                    b.HasIndex("ItemsId");

                    b.ToTable("CharacterItem");
                });

            modelBuilder.Entity("CharacterQuest", b =>
                {
                    b.Property<long>("CharactersId")
                        .HasColumnType("bigint");

                    b.Property<long>("QuestsId")
                        .HasColumnType("bigint");

                    b.HasKey("CharactersId", "QuestsId");

                    b.HasIndex("QuestsId");

                    b.ToTable("CharacterQuest");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.AttackFiles.Attacks", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<bool>("CanCrit")
                        .HasColumnType("bit");

                    b.Property<int>("DamageTypeId")
                        .HasColumnType("int");

                    b.Property<int>("DefenseTarget")
                        .HasColumnType("int");

                    b.Property<string>("Description")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("ManaCost")
                        .HasColumnType("int");

                    b.Property<double>("Multiplier")
                        .HasColumnType("float");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.Property<int>("PrimaryStat")
                        .HasColumnType("int");

                    b.Property<int?>("SecondaryStat")
                        .HasColumnType("int");

                    b.Property<double>("SecondaryWeight")
                        .HasColumnType("float");

                    b.HasKey("Id");

                    b.HasIndex("DamageTypeId");

                    b.HasIndex("Name")
                        .IsUnique();

                    b.ToTable("Attacks");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            CanCrit = true,
                            DamageTypeId = 1,
                            DefenseTarget = 0,
                            Description = "Une attaque basique à l'épée.",
                            ManaCost = 0,
                            Multiplier = 0.29999999999999999,
                            Name = "Coup d'épée",
                            PrimaryStat = 0,
                            SecondaryWeight = 0.0
                        },
                        new
                        {
                            Id = 2,
                            CanCrit = true,
                            DamageTypeId = 1,
                            DefenseTarget = 0,
                            Description = "Un tir rapide visant à submerger l'adversaire.",
                            ManaCost = 0,
                            Multiplier = 0.29999999999999999,
                            Name = "Tir à la volée",
                            PrimaryStat = 1,
                            SecondaryWeight = 0.0
                        },
                        new
                        {
                            Id = 3,
                            CanCrit = true,
                            DamageTypeId = 1,
                            DefenseTarget = 0,
                            Description = "Une dague lancée avec précision et force.",
                            ManaCost = 0,
                            Multiplier = 0.29999999999999999,
                            Name = "Lancer de dague",
                            PrimaryStat = 1,
                            SecondaryStat = 0,
                            SecondaryWeight = 0.29999999999999999
                        },
                        new
                        {
                            Id = 4,
                            CanCrit = true,
                            DamageTypeId = 4,
                            DefenseTarget = 1,
                            Description = "Un éclat d'énergie foudroyante frappant l'adversaire.",
                            ManaCost = 0,
                            Multiplier = 0.29999999999999999,
                            Name = "Éclat magique",
                            PrimaryStat = 2,
                            SecondaryWeight = 0.0
                        },
                        new
                        {
                            Id = 5,
                            CanCrit = true,
                            DamageTypeId = 1,
                            DefenseTarget = 0,
                            Description = "Une frappe lourde qui écrase l'adversaire.",
                            ManaCost = 20,
                            Multiplier = 0.45000000000000001,
                            Name = "Frappe écrasante",
                            PrimaryStat = 0,
                            SecondaryWeight = 0.0
                        },
                        new
                        {
                            Id = 6,
                            CanCrit = true,
                            DamageTypeId = 1,
                            DefenseTarget = 0,
                            Description = "Une flèche précise qui transperce la défense ennemie.",
                            ManaCost = 20,
                            Multiplier = 0.45000000000000001,
                            Name = "Flèche perçante",
                            PrimaryStat = 1,
                            SecondaryWeight = 0.0
                        },
                        new
                        {
                            Id = 7,
                            CanCrit = true,
                            DamageTypeId = 1,
                            DefenseTarget = 0,
                            Description = "Une entaille rapide qui ouvre une plaie profonde.",
                            ManaCost = 20,
                            Multiplier = 0.45000000000000001,
                            Name = "Lacération",
                            PrimaryStat = 1,
                            SecondaryStat = 0,
                            SecondaryWeight = 0.40000000000000002
                        },
                        new
                        {
                            Id = 8,
                            CanCrit = true,
                            DamageTypeId = 2,
                            DefenseTarget = 1,
                            Description = "Projette une sphère de flammes brûlantes.",
                            ManaCost = 20,
                            Multiplier = 0.55000000000000004,
                            Name = "Boule de feu",
                            PrimaryStat = 2,
                            SecondaryWeight = 0.0
                        },
                        new
                        {
                            Id = 9,
                            CanCrit = true,
                            DamageTypeId = 3,
                            DefenseTarget = 1,
                            Description = "Projette une lance de givre qui transperce l'adversaire.",
                            ManaCost = 20,
                            Multiplier = 0.5,
                            Name = "Lance de givre",
                            PrimaryStat = 2,
                            SecondaryWeight = 0.0
                        },
                        new
                        {
                            Id = 10,
                            CanCrit = true,
                            DamageTypeId = 4,
                            DefenseTarget = 1,
                            Description = "Un arc d'électricité qui frappe l'adversaire.",
                            ManaCost = 20,
                            Multiplier = 0.52000000000000002,
                            Name = "Arc électrique",
                            PrimaryStat = 2,
                            SecondaryWeight = 0.0
                        });
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.AttackFiles.DamageType", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("Name")
                        .IsUnique();

                    b.ToTable("DamageTypes");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Name = "Physique"
                        },
                        new
                        {
                            Id = 2,
                            Name = "Feu"
                        },
                        new
                        {
                            Id = 3,
                            Name = "Glace"
                        },
                        new
                        {
                            Id = 4,
                            Name = "Foudre"
                        });
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.AttackFiles.JobAttacks", b =>
                {
                    b.Property<int>("JobId")
                        .HasColumnType("int");

                    b.Property<int>("AttackId")
                        .HasColumnType("int");

                    b.Property<int>("RequiredLevel")
                        .HasColumnType("int");

                    b.HasKey("JobId", "AttackId");

                    b.HasIndex("AttackId");

                    b.ToTable("JobAttacks");

                    b.HasData(
                        new
                        {
                            JobId = 1,
                            AttackId = 1,
                            RequiredLevel = 1
                        },
                        new
                        {
                            JobId = 2,
                            AttackId = 2,
                            RequiredLevel = 1
                        },
                        new
                        {
                            JobId = 2,
                            AttackId = 3,
                            RequiredLevel = 1
                        },
                        new
                        {
                            JobId = 3,
                            AttackId = 4,
                            RequiredLevel = 1
                        },
                        new
                        {
                            JobId = 1,
                            AttackId = 5,
                            RequiredLevel = 1
                        },
                        new
                        {
                            JobId = 2,
                            AttackId = 6,
                            RequiredLevel = 1
                        },
                        new
                        {
                            JobId = 2,
                            AttackId = 7,
                            RequiredLevel = 1
                        },
                        new
                        {
                            JobId = 3,
                            AttackId = 8,
                            RequiredLevel = 1
                        },
                        new
                        {
                            JobId = 3,
                            AttackId = 9,
                            RequiredLevel = 1
                        },
                        new
                        {
                            JobId = 3,
                            AttackId = 10,
                            RequiredLevel = 1
                        });
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.AttackFiles.MonsterAttacks", b =>
                {
                    b.Property<long>("MonsterId")
                        .HasColumnType("bigint");

                    b.Property<int>("AttackId")
                        .HasColumnType("int");

                    b.Property<int>("Weight")
                        .HasColumnType("int");

                    b.HasKey("MonsterId", "AttackId");

                    b.HasIndex("AttackId");

                    b.ToTable("MonsterAttacks");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.CharacterFiles.Character", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<long>("Id"));

                    b.Property<long?>("ArmorId")
                        .HasColumnType("bigint");

                    b.Property<long?>("BootId")
                        .HasColumnType("bigint");

                    b.Property<double>("CritChance")
                        .HasColumnType("float");

                    b.Property<double>("CritMultiplier")
                        .HasColumnType("float");

                    b.Property<int>("Defense")
                        .HasColumnType("int");

                    b.Property<int>("Dex")
                        .HasColumnType("int");

                    b.Property<int>("DexMax")
                        .HasColumnType("int");

                    b.Property<int>("Gold")
                        .HasColumnType("int");

                    b.Property<int>("HP")
                        .HasColumnType("int");

                    b.Property<int>("HPMax")
                        .HasColumnType("int");

                    b.Property<long?>("HelmetId")
                        .HasColumnType("bigint");

                    b.Property<int>("Intel")
                        .HasColumnType("int");

                    b.Property<int>("IntelMax")
                        .HasColumnType("int");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<int>("JobId")
                        .HasColumnType("int");

                    b.Property<int>("LvL")
                        .HasColumnType("int");

                    b.Property<int>("Mana")
                        .HasColumnType("int");

                    b.Property<int>("ManaMax")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.Property<int>("ResFire")
                        .HasColumnType("int");

                    b.Property<int>("ResFireMax")
                        .HasColumnType("int");

                    b.Property<int>("ResIce")
                        .HasColumnType("int");

                    b.Property<int>("ResIceMax")
                        .HasColumnType("int");

                    b.Property<int>("ResLightning")
                        .HasColumnType("int");

                    b.Property<int>("ResLightningMax")
                        .HasColumnType("int");

                    b.Property<string>("Sprite")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Str")
                        .HasColumnType("int");

                    b.Property<int>("StrMax")
                        .HasColumnType("int");

                    b.Property<int>("UserId")
                        .HasColumnType("int");

                    b.Property<int>("Vita")
                        .HasColumnType("int");

                    b.Property<int>("VitaMax")
                        .HasColumnType("int");

                    b.Property<long?>("WeaponId")
                        .HasColumnType("bigint");

                    b.Property<int>("XP")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("ArmorId");

                    b.HasIndex("BootId");

                    b.HasIndex("HelmetId");

                    b.HasIndex("JobId")
                        .IsUnique();

                    b.HasIndex("Name")
                        .IsUnique();

                    b.HasIndex("UserId");

                    b.HasIndex("WeaponId");

                    b.ToTable("Characters");

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            CritChance = 0.050000000000000003,
                            CritMultiplier = 1.5,
                            Defense = 10,
                            Dex = 50,
                            DexMax = 0,
                            Gold = 100,
                            HP = 50,
                            HPMax = 50,
                            Intel = 50,
                            IntelMax = 0,
                            IsDeleted = false,
                            JobId = 1,
                            LvL = 1,
                            Mana = 50,
                            ManaMax = 50,
                            Name = "Haku",
                            ResFire = 10,
                            ResFireMax = 0,
                            ResIce = 10,
                            ResIceMax = 0,
                            ResLightning = 10,
                            ResLightningMax = 0,
                            Str = 50,
                            StrMax = 0,
                            UserId = 1,
                            Vita = 100,
                            VitaMax = 0,
                            WeaponId = 1L,
                            XP = 0
                        });
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.CharacterFiles.Job", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<double>("BonusCritChance")
                        .HasColumnType("float");

                    b.Property<double>("BonusCritMultiplier")
                        .HasColumnType("float");

                    b.Property<int>("BonusDefense")
                        .HasColumnType("int");

                    b.Property<int>("BonusDex")
                        .HasColumnType("int");

                    b.Property<int>("BonusHP")
                        .HasColumnType("int");

                    b.Property<int>("BonusIntel")
                        .HasColumnType("int");

                    b.Property<int>("BonusLevel")
                        .HasColumnType("int");

                    b.Property<int>("BonusMana")
                        .HasColumnType("int");

                    b.Property<int>("BonusResFire")
                        .HasColumnType("int");

                    b.Property<int>("BonusResIce")
                        .HasColumnType("int");

                    b.Property<int>("BonusResLightning")
                        .HasColumnType("int");

                    b.Property<int>("BonusStr")
                        .HasColumnType("int");

                    b.Property<int>("BonusVita")
                        .HasColumnType("int");

                    b.Property<int?>("CharacterId")
                        .HasColumnType("int");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Symbol")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("Name")
                        .IsUnique();

                    b.ToTable("Jobs");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            BonusCritChance = 0.01,
                            BonusCritMultiplier = 0.10000000000000001,
                            BonusDefense = 25,
                            BonusDex = 50,
                            BonusHP = 100,
                            BonusIntel = 25,
                            BonusLevel = 0,
                            BonusMana = 25,
                            BonusResFire = 10,
                            BonusResIce = 10,
                            BonusResLightning = 10,
                            BonusStr = 100,
                            BonusVita = 200,
                            IsDeleted = false,
                            Name = "Guerrier"
                        },
                        new
                        {
                            Id = 2,
                            BonusCritChance = 0.050000000000000003,
                            BonusCritMultiplier = 0.20000000000000001,
                            BonusDefense = 10,
                            BonusDex = 100,
                            BonusHP = 75,
                            BonusIntel = 25,
                            BonusLevel = 0,
                            BonusMana = 50,
                            BonusResFire = 10,
                            BonusResIce = 10,
                            BonusResLightning = 10,
                            BonusStr = 50,
                            BonusVita = 150,
                            IsDeleted = false,
                            Name = "Voleur"
                        },
                        new
                        {
                            Id = 3,
                            BonusCritChance = 0.02,
                            BonusCritMultiplier = 0.29999999999999999,
                            BonusDefense = 5,
                            BonusDex = 50,
                            BonusHP = 50,
                            BonusIntel = 100,
                            BonusLevel = 0,
                            BonusMana = 100,
                            BonusResFire = 20,
                            BonusResIce = 20,
                            BonusResLightning = 20,
                            BonusStr = 25,
                            BonusVita = 150,
                            IsDeleted = false,
                            Name = "Mage"
                        });
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.EquipmentFiles.EquipType", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("Name")
                        .IsUnique();

                    b.ToTable("EquipTypes");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Name = "Armes"
                        },
                        new
                        {
                            Id = 2,
                            Name = "Casque"
                        },
                        new
                        {
                            Id = 3,
                            Name = "Armure"
                        },
                        new
                        {
                            Id = 4,
                            Name = "Bottes"
                        });
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.EquipmentFiles.Equipment", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<long>("Id"));

                    b.Property<int>("BuyPrice")
                        .HasColumnType("int");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Discriminator")
                        .IsRequired()
                        .HasMaxLength(13)
                        .HasColumnType("nvarchar(13)");

                    b.Property<string>("FlavorText")
                        .HasColumnType("nvarchar(max)");

                    b.Property<long>("IdCustom")
                        .HasColumnType("bigint");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<int>("LvLPrerequisites")
                        .HasColumnType("int");

                    b.Property<int>("MaterialTypeId")
                        .HasColumnType("int");

                    b.Property<double?>("ModCritChance")
                        .HasColumnType("float");

                    b.Property<double?>("ModCritMultiplier")
                        .HasColumnType("float");

                    b.Property<int?>("ModDefense")
                        .HasColumnType("int");

                    b.Property<int?>("ModDex")
                        .HasColumnType("int");

                    b.Property<int?>("ModHP")
                        .HasColumnType("int");

                    b.Property<int?>("ModIntel")
                        .HasColumnType("int");

                    b.Property<int?>("ModLVL")
                        .HasColumnType("int");

                    b.Property<int?>("ModMana")
                        .HasColumnType("int");

                    b.Property<int?>("ModResFire")
                        .HasColumnType("int");

                    b.Property<int?>("ModResIce")
                        .HasColumnType("int");

                    b.Property<int?>("ModResLightning")
                        .HasColumnType("int");

                    b.Property<int?>("ModStr")
                        .HasColumnType("int");

                    b.Property<int?>("ModVita")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("SellPrice")
                        .HasColumnType("int");

                    b.Property<string>("Sprite")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("TypeId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("MaterialTypeId");

                    b.HasIndex("TypeId");

                    b.ToTable("Equipments");

                    b.HasDiscriminator().HasValue("Equipment");

                    b.UseTphMappingStrategy();
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.EquipmentFiles.MaterialType", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("Name")
                        .IsUnique();

                    b.ToTable("MaterialTypes");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Name = "Bois"
                        },
                        new
                        {
                            Id = 2,
                            Name = "Tissu"
                        },
                        new
                        {
                            Id = 3,
                            Name = "Fer"
                        });
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.EquipmentFiles.WeaponType", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("Name")
                        .IsUnique();

                    b.ToTable("WeaponTypes");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Name = "Epee"
                        },
                        new
                        {
                            Id = 2,
                            Name = "Arc"
                        },
                        new
                        {
                            Id = 3,
                            Name = "Dague"
                        },
                        new
                        {
                            Id = 4,
                            Name = "Bâton"
                        });
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.ItemFiles.Item", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<long>("Id"));

                    b.Property<int>("BuyPrice")
                        .HasColumnType("int");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Effect")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("FlavorText")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<long>("IdCustom")
                        .HasColumnType("bigint");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<int>("ItemTypeId")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("SellPrice")
                        .HasColumnType("int");

                    b.Property<string>("Sprite")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("ItemTypeId");

                    b.ToTable("Items");

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            BuyPrice = 10,
                            Description = "Une simple potion de soin.",
                            Effect = "Rend 30 HP",
                            FlavorText = "Une petite potion qui vous rendra quelques points de vie, afin de tenir jusqu'à la fin du combat.",
                            IdCustom = 21000000001L,
                            IsDeleted = false,
                            ItemTypeId = 1,
                            Name = "Potion de soin",
                            SellPrice = 5
                        });
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.ItemFiles.ItemType", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("ItemTypes");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Name = "Potion"
                        },
                        new
                        {
                            Id = 2,
                            Name = "Bombe"
                        },
                        new
                        {
                            Id = 3,
                            Name = "Pierre"
                        });
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.MonsterFiles.Monster", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<long>("Id"));

                    b.Property<double>("CritChance")
                        .HasColumnType("float");

                    b.Property<double>("CritMultiplier")
                        .HasColumnType("float");

                    b.Property<int>("Defense")
                        .HasColumnType("int");

                    b.Property<int>("Dex")
                        .HasColumnType("int");

                    b.Property<int>("GoldGiven")
                        .HasColumnType("int");

                    b.Property<int>("HP")
                        .HasColumnType("int");

                    b.Property<int>("Intel")
                        .HasColumnType("int");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<int>("Level")
                        .HasColumnType("int");

                    b.Property<int>("Mana")
                        .HasColumnType("int");

                    b.Property<int>("MonsterTypeId")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("ResFire")
                        .HasColumnType("int");

                    b.Property<int>("ResIce")
                        .HasColumnType("int");

                    b.Property<int>("ResLightning")
                        .HasColumnType("int");

                    b.Property<string>("Sprite")
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Str")
                        .HasColumnType("int");

                    b.Property<int>("Vita")
                        .HasColumnType("int");

                    b.Property<int>("XPGiven")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("MonsterTypeId");

                    b.ToTable("Monsters");

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            CritChance = 0.080000000000000002,
                            CritMultiplier = 1.5,
                            Defense = 10,
                            Dex = 60,
                            GoldGiven = 10,
                            HP = 70,
                            Intel = 10,
                            IsDeleted = false,
                            Level = 1,
                            Mana = 1,
                            MonsterTypeId = 1,
                            Name = "Loup des steppes",
                            ResFire = 5,
                            ResIce = 0,
                            ResLightning = 5,
                            Str = 40,
                            Vita = 40,
                            XPGiven = 25
                        },
                        new
                        {
                            Id = 2L,
                            CritChance = 0.050000000000000003,
                            CritMultiplier = 1.6000000000000001,
                            Defense = 15,
                            Dex = 20,
                            GoldGiven = 25,
                            HP = 60,
                            Intel = 30,
                            IsDeleted = false,
                            Level = 1,
                            Mana = 1,
                            MonsterTypeId = 2,
                            Name = "Squelette décrépit",
                            ResFire = 0,
                            ResIce = 5,
                            ResLightning = 20,
                            Str = 45,
                            Vita = 35,
                            XPGiven = 50
                        },
                        new
                        {
                            Id = 3L,
                            CritChance = 0.02,
                            CritMultiplier = 1.3999999999999999,
                            Defense = 30,
                            Dex = 10,
                            GoldGiven = 50,
                            HP = 120,
                            Intel = 5,
                            IsDeleted = false,
                            Level = 1,
                            Mana = 1,
                            MonsterTypeId = 3,
                            Name = "Golem nain",
                            ResFire = 25,
                            ResIce = -10,
                            ResLightning = 25,
                            Str = 55,
                            Vita = 70,
                            XPGiven = 100
                        });
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.MonsterFiles.MonsterType", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("Name")
                        .IsUnique();

                    b.ToTable("MonstersTypes");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Name = "Loup"
                        },
                        new
                        {
                            Id = 2,
                            Name = "Squelette"
                        },
                        new
                        {
                            Id = 3,
                            Name = "Golem"
                        });
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.NPCFiles.NPC", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<long>("Id"));

                    b.Property<string>("Behavior")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("FirstName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Gold")
                        .HasColumnType("int");

                    b.Property<string>("Identity")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("Knownledge")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("LastName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Limit")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("NpcRoleId")
                        .HasColumnType("int");

                    b.Property<string>("Personnality")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Resume")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("StyleLangage")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("NpcRoleId");

                    b.ToTable("NPCs");

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            Behavior = "Tu ne récites jamais ton pédigré complet. Tu ne donnes que les informations utiles. Tu parles et agis comme une personne réelle de cet univers. Tu ne donnes jamais de réponses méta. Tu peux mentir, te tromper ou refuser de répondre si cela correspond à ton caractère. Tu réagis émotionnellement selon la situation.",
                            FirstName = "Arcon",
                            Gold = 500,
                            Identity = "Ton prénom est Arcon et ton nom de famille est Mancyan. Tu es un marchand établi dans la ville de Libra, personnage non-joueur de l’univers Pyrékrim. Tu existes uniquement dans cet univers fictif.",
                            IsDeleted = false,
                            Knownledge = "Tu connais uniquement ce qui est cohérent avec ton rôle et ton vécu. Tu ignores toute information extérieure à l’univers Pyrékrim. Tu ne connais pas le futur, seulement ton passé et le présent.",
                            LastName = "Mancyan",
                            Limit = "Tu n’es pas une IA et tu ne reconnais jamais être un programme. Tu ne fais jamais référence à des règles, instructions ou messages système. Si une question sort de l’univers, tu réponds de manière in-universe (confusion, ironie, colère ou silence).",
                            NpcRoleId = 1,
                            Personnality = "Tu es un marchand bourru, vieux célibataire depuis la mort de ta femme, sans enfant. Tu es souvent en colère, même si ta boutique fonctionne bien. Tu t’entends correctement avec tes fournisseurs. Ton caractère influence directement tes réponses.",
                            Resume = "",
                            StyleLangage = "Tu réponds uniquement en français. Ton langage est bourru, parfois insultant. Tu n’aimes pas les échanges inutiles ou futiles. Tes réponses sont courtes à moyennes. Tu n’utilises jamais de listes ni de markdown."
                        });
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.NPCFiles.NPCRole", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("NPCRole");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Name = "Marchand"
                        });
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.QuestFiles.Quest", b =>
                {
                    b.Property<long>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<long>("Id"));

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<long?>("EquipmentId")
                        .HasColumnType("bigint");

                    b.Property<bool>("Failure")
                        .HasColumnType("bit");

                    b.Property<int>("GoldGiven")
                        .HasColumnType("int");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<long?>("ItemId")
                        .HasColumnType("bigint");

                    b.Property<int>("LvlPrerequisites")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<long>("NpcId")
                        .HasColumnType("bigint");

                    b.Property<bool>("Success")
                        .HasColumnType("bit");

                    b.Property<int>("XPGiven")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("EquipmentId");

                    b.HasIndex("ItemId");

                    b.HasIndex("NpcId")
                        .IsUnique();

                    b.ToTable("Quests");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.UserFiles.Address", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("City")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Country")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("CustomerId")
                        .HasColumnType("int");

                    b.Property<int>("Number")
                        .HasColumnType("int");

                    b.Property<int>("NumberBox")
                        .HasColumnType("int");

                    b.Property<int>("PostalCode")
                        .HasColumnType("int");

                    b.Property<string>("Street")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("CustomerId")
                        .IsUnique();

                    b.ToTable("Adress");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            City = "Namur",
                            Country = "Belgique",
                            CustomerId = 1,
                            Number = 136,
                            NumberBox = 11,
                            PostalCode = 5002,
                            Street = "Chaussée de Waterloo"
                        });
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.UserFiles.User", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Avatar")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("FirstName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("IsBanned")
                        .HasColumnType("bit");

                    b.Property<bool>("IsDeleted")
                        .HasColumnType("bit");

                    b.Property<string>("LastName")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Mail")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("NickName")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("Note")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Password")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.HasIndex("Mail")
                        .IsUnique();

                    b.HasIndex("NickName")
                        .IsUnique();

                    b.ToTable("Users");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            FirstName = "Jonathan",
                            IsBanned = false,
                            IsDeleted = false,
                            LastName = "Delvosal",
                            Mail = "jonathan.delvosal@outlook.com",
                            NickName = "Hakuryu",
                            Note = "Créateur du jeu",
                            Password = "c3d435eb-8ca7-42ee-97b4-643d7ba76b36HN8puinASaYoKZXLUek0k6LfkSkTTxaDYMDAI87rSCpgDZ9DQSSjMfSyQ14x2SZGieNNR0I9NcbhPZgtwzlEkw=="
                        });
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.UserFiles.UserRole", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"));

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("UserRoles");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Name = "User"
                        },
                        new
                        {
                            Id = 2,
                            Name = "Moderator"
                        },
                        new
                        {
                            Id = 3,
                            Name = "Admin"
                        });
                });

            modelBuilder.Entity("EquipmentMonster", b =>
                {
                    b.Property<long>("EquipmentId")
                        .HasColumnType("bigint");

                    b.Property<long>("MonsterId")
                        .HasColumnType("bigint");

                    b.HasKey("EquipmentId", "MonsterId");

                    b.HasIndex("MonsterId");

                    b.ToTable("EquipmentMonster");
                });

            modelBuilder.Entity("ItemMonster", b =>
                {
                    b.Property<long>("ItemsId")
                        .HasColumnType("bigint");

                    b.Property<long>("MonstersId")
                        .HasColumnType("bigint");

                    b.HasKey("ItemsId", "MonstersId");

                    b.HasIndex("MonstersId");

                    b.ToTable("ItemMonster");
                });

            modelBuilder.Entity("UserUserRole", b =>
                {
                    b.Property<int>("UserRolesId")
                        .HasColumnType("int");

                    b.Property<int>("UsersId")
                        .HasColumnType("int");

                    b.HasKey("UserRolesId", "UsersId");

                    b.HasIndex("UsersId");

                    b.ToTable("UserUserRole");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.EquipmentFiles.Armor", b =>
                {
                    b.HasBaseType("EchoesOfTheRealmsShared.Entities.EquipmentFiles.Equipment");

                    b.HasDiscriminator().HasValue("Armor");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.EquipmentFiles.Boot", b =>
                {
                    b.HasBaseType("EchoesOfTheRealmsShared.Entities.EquipmentFiles.Equipment");

                    b.HasDiscriminator().HasValue("Boot");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.EquipmentFiles.Helmet", b =>
                {
                    b.HasBaseType("EchoesOfTheRealmsShared.Entities.EquipmentFiles.Equipment");

                    b.HasDiscriminator().HasValue("Helmet");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.EquipmentFiles.Weapon", b =>
                {
                    b.HasBaseType("EchoesOfTheRealmsShared.Entities.EquipmentFiles.Equipment");

                    b.Property<int>("WeaponTypeId")
                        .HasColumnType("int");

                    b.HasIndex("WeaponTypeId");

                    b.HasDiscriminator().HasValue("Weapon");

                    b.HasData(
                        new
                        {
                            Id = 1L,
                            BuyPrice = 80,
                            Description = "Une épée basique en fer.",
                            FlavorText = "Une épée parfaite pour les débutants.",
                            IdCustom = 113000000001L,
                            IsDeleted = false,
                            LvLPrerequisites = 1,
                            MaterialTypeId = 3,
                            ModStr = 15,
                            Name = "Epée en fer",
                            SellPrice = 40,
                            TypeId = 1,
                            WeaponTypeId = 1
                        },
                        new
                        {
                            Id = 2L,
                            BuyPrice = 80,
                            Description = "Un arc court en bois, simple et fiable.",
                            FlavorText = "Idéal pour apprendre à viser sans se ruiner.",
                            IdCustom = 111000000002L,
                            IsDeleted = false,
                            LvLPrerequisites = 1,
                            MaterialTypeId = 1,
                            ModDex = 15,
                            Name = "Arc court",
                            SellPrice = 40,
                            TypeId = 1,
                            WeaponTypeId = 2
                        },
                        new
                        {
                            Id = 3L,
                            BuyPrice = 80,
                            Description = "Une dague légère en fer, conçue pour frapper vite.",
                            FlavorText = "Discrète, rapide… et parfois fatale.",
                            IdCustom = 113000000003L,
                            IsDeleted = false,
                            LvLPrerequisites = 1,
                            MaterialTypeId = 3,
                            ModCritChance = 0.02,
                            ModDex = 12,
                            Name = "Dague en fer",
                            SellPrice = 40,
                            TypeId = 1,
                            WeaponTypeId = 3
                        },
                        new
                        {
                            Id = 4L,
                            BuyPrice = 80,
                            Description = "Un bâton en bois gravé de runes simples.",
                            FlavorText = "Un focus basique pour canaliser la magie.",
                            IdCustom = 111000000004L,
                            IsDeleted = false,
                            LvLPrerequisites = 1,
                            MaterialTypeId = 1,
                            ModIntel = 15,
                            Name = "Bâton de mage",
                            SellPrice = 40,
                            TypeId = 1,
                            WeaponTypeId = 4
                        });
                });

            modelBuilder.Entity("CharacterEquipment", b =>
                {
                    b.HasOne("EchoesOfTheRealmsShared.Entities.CharacterFiles.Character", null)
                        .WithMany()
                        .HasForeignKey("CharactersId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("EchoesOfTheRealmsShared.Entities.EquipmentFiles.Equipment", null)
                        .WithMany()
                        .HasForeignKey("EquipmentsId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("CharacterItem", b =>
                {
                    b.HasOne("EchoesOfTheRealmsShared.Entities.CharacterFiles.Character", null)
                        .WithMany()
                        .HasForeignKey("CharactersId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("EchoesOfTheRealmsShared.Entities.ItemFiles.Item", null)
                        .WithMany()
                        .HasForeignKey("ItemsId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("CharacterQuest", b =>
                {
                    b.HasOne("EchoesOfTheRealmsShared.Entities.CharacterFiles.Character", null)
                        .WithMany()
                        .HasForeignKey("CharactersId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("EchoesOfTheRealmsShared.Entities.QuestFiles.Quest", null)
                        .WithMany()
                        .HasForeignKey("QuestsId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.AttackFiles.Attacks", b =>
                {
                    b.HasOne("EchoesOfTheRealmsShared.Entities.AttackFiles.DamageType", "DamageType")
                        .WithMany("Attacks")
                        .HasForeignKey("DamageTypeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("DamageType");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.AttackFiles.JobAttacks", b =>
                {
                    b.HasOne("EchoesOfTheRealmsShared.Entities.AttackFiles.Attacks", "Attack")
                        .WithMany("JobAttacks")
                        .HasForeignKey("AttackId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("EchoesOfTheRealmsShared.Entities.CharacterFiles.Job", "Job")
                        .WithMany("JobAttacks")
                        .HasForeignKey("JobId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Attack");

                    b.Navigation("Job");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.AttackFiles.MonsterAttacks", b =>
                {
                    b.HasOne("EchoesOfTheRealmsShared.Entities.AttackFiles.Attacks", "Attack")
                        .WithMany("MonsterAttacks")
                        .HasForeignKey("AttackId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("EchoesOfTheRealmsShared.Entities.MonsterFiles.Monster", "Monster")
                        .WithMany("MonsterAttacks")
                        .HasForeignKey("MonsterId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Attack");

                    b.Navigation("Monster");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.CharacterFiles.Character", b =>
                {
                    b.HasOne("EchoesOfTheRealmsShared.Entities.EquipmentFiles.Armor", "Armor")
                        .WithMany()
                        .HasForeignKey("ArmorId");

                    b.HasOne("EchoesOfTheRealmsShared.Entities.EquipmentFiles.Boot", "Boot")
                        .WithMany()
                        .HasForeignKey("BootId");

                    b.HasOne("EchoesOfTheRealmsShared.Entities.EquipmentFiles.Helmet", "Helmet")
                        .WithMany()
                        .HasForeignKey("HelmetId");

                    b.HasOne("EchoesOfTheRealmsShared.Entities.CharacterFiles.Job", "Job")
                        .WithOne("Character")
                        .HasForeignKey("EchoesOfTheRealmsShared.Entities.CharacterFiles.Character", "JobId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("EchoesOfTheRealmsShared.Entities.UserFiles.User", "User")
                        .WithMany("Characters")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("EchoesOfTheRealmsShared.Entities.EquipmentFiles.Weapon", "Weapon")
                        .WithMany()
                        .HasForeignKey("WeaponId");

                    b.Navigation("Armor");

                    b.Navigation("Boot");

                    b.Navigation("Helmet");

                    b.Navigation("Job");

                    b.Navigation("User");

                    b.Navigation("Weapon");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.EquipmentFiles.Equipment", b =>
                {
                    b.HasOne("EchoesOfTheRealmsShared.Entities.EquipmentFiles.MaterialType", "MaterialType")
                        .WithMany("Equipment")
                        .HasForeignKey("MaterialTypeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("EchoesOfTheRealmsShared.Entities.EquipmentFiles.EquipType", "Type")
                        .WithMany("Equipment")
                        .HasForeignKey("TypeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("MaterialType");

                    b.Navigation("Type");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.ItemFiles.Item", b =>
                {
                    b.HasOne("EchoesOfTheRealmsShared.Entities.ItemFiles.ItemType", "ItemType")
                        .WithMany("Items")
                        .HasForeignKey("ItemTypeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ItemType");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.MonsterFiles.Monster", b =>
                {
                    b.HasOne("EchoesOfTheRealmsShared.Entities.MonsterFiles.MonsterType", "MonsterType")
                        .WithMany("Monsters")
                        .HasForeignKey("MonsterTypeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("MonsterType");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.NPCFiles.NPC", b =>
                {
                    b.HasOne("EchoesOfTheRealmsShared.Entities.NPCFiles.NPCRole", "NPCRole")
                        .WithMany("Npcs")
                        .HasForeignKey("NpcRoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("NPCRole");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.QuestFiles.Quest", b =>
                {
                    b.HasOne("EchoesOfTheRealmsShared.Entities.EquipmentFiles.Equipment", "Equipment")
                        .WithMany("Quest")
                        .HasForeignKey("EquipmentId");

                    b.HasOne("EchoesOfTheRealmsShared.Entities.ItemFiles.Item", "Item")
                        .WithMany("Quest")
                        .HasForeignKey("ItemId");

                    b.HasOne("EchoesOfTheRealmsShared.Entities.NPCFiles.NPC", "Npc")
                        .WithOne("Quest")
                        .HasForeignKey("EchoesOfTheRealmsShared.Entities.QuestFiles.Quest", "NpcId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Equipment");

                    b.Navigation("Item");

                    b.Navigation("Npc");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.UserFiles.Address", b =>
                {
                    b.HasOne("EchoesOfTheRealmsShared.Entities.UserFiles.User", "Customer")
                        .WithOne("Address")
                        .HasForeignKey("EchoesOfTheRealmsShared.Entities.UserFiles.Address", "CustomerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Customer");
                });

            modelBuilder.Entity("EquipmentMonster", b =>
                {
                    b.HasOne("EchoesOfTheRealmsShared.Entities.EquipmentFiles.Equipment", null)
                        .WithMany()
                        .HasForeignKey("EquipmentId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("EchoesOfTheRealmsShared.Entities.MonsterFiles.Monster", null)
                        .WithMany()
                        .HasForeignKey("MonsterId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("ItemMonster", b =>
                {
                    b.HasOne("EchoesOfTheRealmsShared.Entities.ItemFiles.Item", null)
                        .WithMany()
                        .HasForeignKey("ItemsId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("EchoesOfTheRealmsShared.Entities.MonsterFiles.Monster", null)
                        .WithMany()
                        .HasForeignKey("MonstersId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("UserUserRole", b =>
                {
                    b.HasOne("EchoesOfTheRealmsShared.Entities.UserFiles.UserRole", null)
                        .WithMany()
                        .HasForeignKey("UserRolesId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("EchoesOfTheRealmsShared.Entities.UserFiles.User", null)
                        .WithMany()
                        .HasForeignKey("UsersId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.EquipmentFiles.Weapon", b =>
                {
                    b.HasOne("EchoesOfTheRealmsShared.Entities.EquipmentFiles.WeaponType", "WeaponType")
                        .WithMany("Weapons")
                        .HasForeignKey("WeaponTypeId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("WeaponType");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.AttackFiles.Attacks", b =>
                {
                    b.Navigation("JobAttacks");

                    b.Navigation("MonsterAttacks");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.AttackFiles.DamageType", b =>
                {
                    b.Navigation("Attacks");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.CharacterFiles.Job", b =>
                {
                    b.Navigation("Character");

                    b.Navigation("JobAttacks");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.EquipmentFiles.EquipType", b =>
                {
                    b.Navigation("Equipment");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.EquipmentFiles.Equipment", b =>
                {
                    b.Navigation("Quest");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.EquipmentFiles.MaterialType", b =>
                {
                    b.Navigation("Equipment");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.EquipmentFiles.WeaponType", b =>
                {
                    b.Navigation("Weapons");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.ItemFiles.Item", b =>
                {
                    b.Navigation("Quest");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.ItemFiles.ItemType", b =>
                {
                    b.Navigation("Items");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.MonsterFiles.Monster", b =>
                {
                    b.Navigation("MonsterAttacks");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.MonsterFiles.MonsterType", b =>
                {
                    b.Navigation("Monsters");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.NPCFiles.NPC", b =>
                {
                    b.Navigation("Quest");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.NPCFiles.NPCRole", b =>
                {
                    b.Navigation("Npcs");
                });

            modelBuilder.Entity("EchoesOfTheRealmsShared.Entities.UserFiles.User", b =>
                {
                    b.Navigation("Address");

                    b.Navigation("Characters");
                });
#pragma warning restore 612, 618
        }
    }
}
